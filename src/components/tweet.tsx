...
const TweetControlBtn = styled.svg`
  width: 20px;
  height: 20px;
  cursor: pointer;
`;
...
export default function Tweet({ username, userId, photo, tweet, id }: ITweet) {
  ...
  const onEdit = () => {
    ...
    setShowEditForm({ tweetId: id, showEdit: true });
  };
  ...
  return (
    <>
      {showEditForm.tweetId === id && showEditForm.showEdit ? (
        <EditWrapper>
          <CloseBtnWrapper>
            <CloseBtn
          ...
          <EditTweetForm id={id} tweet={tweet} userId={userId} photo={photo} />
        </EditWrapper>
      ) : (
        <Wrapper>
          <Row>
